...
export default class Home extends Component {
    ...
    async handleUploadSuccess (filename) {
        ...
        try {
            let { bucket, fullPath } = await firebase.storage().ref(imageRef).child(filename).getMetadata();
            console.log('bucket', bucket)
            console.log('fullPath', fullPath)
            let downloadURL = await firebase.storage().ref(imageRef).child(filename).getDownloadURL();
